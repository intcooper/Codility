/**
 * @brief Solution to the _Dinner_ task published as part of the Codility Year of The Rabbit challenge.
 *
 * There are N people sitting at a round table, having dinner. 
 * They are numbered from 0 to N-1 in clockwise order. Initially, each person has a dish on the table in front of them. 
 ...
 * 
 * that, given arrays A and B, both consisting of N integers, representing tastes that people do not like and 
 * the taste of their dishes, respectively, returns the minimum number of table rotations required so that every person 
 * has a dish whose taste they do not dislike. 
 * In particular, if no rotations are needed, the function should return 0. If fulfilling the above condition is impossible, 
